...
<div class="mx-1 px-2 my-5 position-relative">
    ...
    @if (strlen($judul) > 30)
        <div class="judul-container d-flex justify-content-between">
            ...
            <a class="btn lihat-selengkapnya float-end" onclick="showFullText(this)"><i class="bi bi-caret-down-fill"></i></a>
            <a class="btn tutup float-end" style="display: none;" onclick="hideFullText(this)"><i class="bi bi-caret-up-fill"></i></a>
        </div>
    @else
        <p>{{ $judul }}</p>
    @endif
    ...
    <a href='{{ route('ekskul.show', $profils->nama) }}' class="btn position-absolute top-0 start-0 translate-middle ms-4"><i class="bi bi-arrow-left-square-fill fs-1 text-dark"></i></a>
</div>
